...
@section('content')
    ...
    <form action="{{route('artCommentUpdate')}}" method="post" id="art_form" enctype="multipart/form-data">
        @csrf
        <div class="form-group">
            <label class="sr-only" for="inputName">Hidden input label</label>
            <input type="hidden" class="form-control" value="{{$artComment['id']}}" name="ac_id" id="inputName" placeholder="">
        </div>
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <label for="">内容</label>
                    <input type="text" class="form-control" value="{{$artComment['comment']}}" id="inputName" placeholder="">
                    <script id="editor" name="acContent" value="" type="text/plain" style="width:1000px;height:200px;">
                    </script>
                </div>
            </div>
            <div class="card-footer text-muted"></div>
            <div class="card mt-2">
                <input type="submit" value='修改' class="btn btn-success mr-2">
            </div>
        </div>
    </form>
